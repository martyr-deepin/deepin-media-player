...
class HScalebar(gtk.HScale):
    ...
    def expose_h_scalebar(self, widget, event):
        ...
        # Draw background.
        draw_pixbuf(cr, left_bg_pixbuf, x, y)
        draw_pixbuf(cr, middle_bg_pixbuf.scale_simple(w - side_width * 2, line_height, gtk.gdk.INTERP_BILINEAR), x + side_width, y)
        draw_pixbuf(cr, right_bg_pixbuf, x + w - side_width, y)
        
        # Draw foreground.
        if value > 0:
            draw_pixbuf(cr, left_fg_pixbuf, x, y)
            draw_pixbuf(cr, middle_fg_pixbuf.scale_simple(value, line_height, gtk.gdk.INTERP_BILINEAR), x + side_width, y)
            draw_pixbuf(cr, right_fg_pixbuf, x + value, y)
            
        # Draw drag point.
        # draw_pixbuf(cr, point_pixbuf, x + value - point_pixbuf.get_width() / 2, y-5)    
        draw_pixbuf(cr, point_pixbuf, x + value - point_pixbuf.get_width() / 2, y-5)    
